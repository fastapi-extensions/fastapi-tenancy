...
Design note
-----------
``_sanitize_identifier`` is imported from ``fastapi_tenancy.utils._sanitize``,
a dependency-free internal module that both ``db_compat`` and ``utils.validation``
can import without creating circular dependency chains.
"""
...
from fastapi_tenancy.utils._sanitize import (
    core_sanitize_identifier as _sanitize_identifier,
)
...
# _sanitize_identifier is imported from fastapi_tenancy.utils._sanitize at the
# top of this module.  The shared implementation eliminates the duplication
# that previously existed between this module and utils.validation.

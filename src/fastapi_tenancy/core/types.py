...
Lazy re-exports
    ``BaseTenantResolver`` and ``BaseIsolationProvider`` are imported lazily
    via ``__getattr__`` to break the circular import that would arise if they
    were imported at module load time (those modules import ``Tenant`` from
    here, so importing them here would be circular).
"""
...
def __getattr__(name: str) -> Any:
    ...
    ``BaseTenantResolver`` and ``BaseIsolationProvider`` both import from
    this module, so importing them here at module-load time would create a
    circular import.  Deferring to first attribute access breaks the cycle
    while still exposing them via ``fastapi_tenancy.core.types.*``.
    ...
    if name == "BaseTenantResolver":
        from fastapi_tenancy.resolution.base import BaseTenantResolver  # noqa: PLC0415

        return BaseTenantResolver

    if name == "BaseIsolationProvider":
        from fastapi_tenancy.isolation.base import BaseIsolationProvider  # noqa: PLC0415

...
* :func:`assert_safe_schema_name` and :func:`assert_safe_database_name`
  raise immediately on invalid input — never silently truncate or sanitise.
  Callers that *want* sanitisation should use :func:`sanitize_identifier`.
"""
...
# Tenant slug: lowercase letter, then 1-61 letters/digits/hyphens, then
# alphanumeric — total 3-63 characters.
...
def validate_tenant_identifier(identifier: str) -> bool:
    ...
    * Contains only lowercase letters, digits, and hyphens in between.
    ...
    The length is checked *before* the regular expression to prevent
    ReDoS on adversarially crafted inputs longer than 512 bytes.
...
def validate_database_name(database_name: str) -> bool:
    ...
    Uses identical rules to :func:`validate_schema_name` since PostgreSQL
    applies the same grammar to both schema and database names.
...
def assert_safe_schema_name(schema_name: str, *, context: str = "") -> None:
    """Raise :class:`ValueError` if *schema_name* is not a safe identifier.

    Call this immediately before any DDL statement that interpolates a
    schema name.  Raising is the correct behaviour — never silently
    truncate or modify the value.

    Args:
        schema_name: The schema name to assert is safe.
        context: Optional human-readable call-site description included in
...
def assert_safe_database_name(database_name: str, *, context: str = "") -> None:
    """Raise :class:`ValueError` if *database_name* is not a safe identifier.
...
def sanitize_identifier(identifier: str) -> str:
    ...
    This function is *lossy* — it normalises the input rather than
    rejecting invalid values.  Use :func:`assert_safe_schema_name` when
    you need a strict guard; use this function only when producing
    derived values such as schema-name prefixes and database names from
    user-supplied slugs.
...
def validate_json_serializable(value: Any) -> bool:
    ...
    Returns:
        ``True`` when :func:`json.dumps` succeeds without error.
    """
    try:
        json.dumps(value)
        return True
